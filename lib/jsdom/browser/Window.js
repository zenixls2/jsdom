...
function Window(options) {
  ...
  this._defaultLocation = idlUtils.implForWrapper(this._document)._location;
  this._defaultHistory = idlUtils.implForWrapper(this._document)._history;
  ...
  define(this, {
    ...
    get location() {
      let wrapper = idlUtils.implForWrapper(window._document);
      if (wrapper) {
        return idlUtils.wrapperForImpl(wrapper._location);
      }
      return window._defaultLocation;
    },
    get history() {
      let wrapper = idlUtils.implForWrapper(window._document);
      if (wrapper) {
        return idlUtils.wrapperForImpl(wrapper._history);
      }
      return window._defaultHistory;
    },
    get navigator() {
      return navigator;
    }
  ...
  this.close = function () {
    ...
    if (this._document) {
      ...
      let queue = idlUtils.implForWrapper(this._document)._queue;
      queue.paused = true;
      let head = queue.tail;
...
function startTimer(window, startFn, stopFn, timerId, callback, ms, args) {
  ...
  if (res) {
    res.timerId = id;
    window.__timers[id] = [res, stopFn];
  }
  return res;
}

function stopTimer(window, res) {
  const timer = window.__timers[res.timerId];
  if (timer) {
    // Need to .call() with undefined to ensure the thisArg is not timer itself
    timer[1].call(undefined, timer[0]);
    delete window.__timers[res.timerId];
  }
}

function stopAllTimers(window) {
  Object.getOwnPropertySymbols(window.__timers).forEach(key => {
