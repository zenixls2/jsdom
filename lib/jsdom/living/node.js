...
// dirty hack from dom-token-list implementation

const INTERNAL = Symbol.for("DOMTokenList internal");
module.exports.listOfElementsWithClassNames = (classNames, root) => {
  ...
  return createHTMLCollection(root, () => {
    ...
    const caseInsensitive = isQuirksMode;
    const classes = caseInsensitive ? orderedSetParser(classNames.toLowerCase()): orderedSetParser(classNames);
    if (classes.size == 0) {
      return []
    }
    return domSymbolTree.treeToArray(root, { filter(node) {
      ...
      const tokenSet = new Set(node.classList[INTERNAL].tokens);

      if (tokenSet.size < classes.size) return false;
      for (const className of classes) {
        if (!tokenSet.has(className)) {
          return false;
        }
      }
      return true;
