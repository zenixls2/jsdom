...
module.exports = function createXMLHttpRequest(window) {
  ...
  let filter = {
    ...
    defineProperty(target, property, descriptor) {
      return req.defineProperty(property);
    },
    setPrototypeOf(target, prototype) {
      req.setPrototypeOf(prototype);
      return true;
    },
    has(target, prop) {
      return (prop in req);
    ...
    apply(target, thisArg, argumentsList) {
      return target.apply(req, argumentsList);
    }
  };
  let dummyConstructor = function() {
    return new Proxy(req, filter)
  }
...
function extractBody(bodyInit) {
  ...
  if (Blob.isImpl(bodyInit)) {
    ...
    return {
      buffer: Buffer.from(new Uint8Array(bodyInit)),
      contentType: null
    };
