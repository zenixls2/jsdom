...
function attachName(name, elm, doc) {
  if (name && elm && doc) {
    if (!doc._names[name]) {
      doc._names[name] = []
    }
    doc._names[name].push(elm);
  }
}
...
function detachName(name, elm, doc) {
  if (name && elm && doc) {
    if (doc._names && doc._names[name]) {
      const elms = doc._names[name];
      for (let i = 0; i < elms.length; i++) {
        if (elms[i] == elm) {
          elms.splice(i, 1);
          --i;
...
class ElementImpl extends NodeImpl {
  ...
  _attach() {
    ...
    if (name) {
      attachName(name, this, this._ownerDocument);
    }
  ...
  _detach() {
    ...
    const name = this.getAttribute('name');
    if (name) {
      detachName(name, this, this._ownerDocument);
    }
  }
  ...
  _attrModified(name, value, oldValue) {
    ...
    if (this._attached) {
      if (name === "id") {
        ...
        const doc = this._ownerDocument;
        detachName(oldValue, this, doc);
        attachName(value, this, doc);
      }
    }
...
// should design to invalid the cache somewhere
