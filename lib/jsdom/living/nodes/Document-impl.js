...
class DocumentImpl extends NodeImpl {
  ...
  get domain() {
    return this._domain ? this._domain : (this.location || {}).hostname;
  }

  set domain(value) {
    let hostname = (this.location || {}).hostname
    if (hostname && hostname.indexOf(value) > 0) {
      this._domain = value;
    }
  }
  ...
  get location() {
    return this._defaultView ? this._location : Location.createImpl([], { relevantDocument: this });
  }
  get documentElement() {
  ...
  write() {
    ...
    if (this._currentScript != null && this._currentScript.async == true) {
      this._defaultView.console.error("A call to document.write() from an asynchronously-loaded external script was ignored");
      return;
    }
    ...
    if (this._writeAfterElement) {
      ...
      // get current running item
      let head = this._queue.tail;
      while (head && head.prev) {
        head = head.prev;
      }
      // split later items
      ...
      while (child) {
        ...
        // maybe when write created script invoked write.
        // recursive write? TO-CHECK
        if (!parent._attached && node._attach) {
          // if parent is attached, this would not be executed
          node._attach();
        }
        previous = node;
      }

      // put back later items to resource queue
      if (head) {
        if (save) {
          save.prev = this._queue.tail;
        }
        if (this._queue.tail) {
          this._queue.tail.next = save;
        }
        this._queue.tail = saveTail;
      }
  ...
  open() {
    if (this._currentScript != null && this._currentScript.src != null) {
      let url = new URL(this._currentScript.src, this._origin);
      if (url.origin != this._origin) {
        throw new DOMException(DOMException.INVALID_ACCESS_ERR,
          "document.open failed due to violation of the same-origin policy");
        return this;
      }
    }
  ...
  getElementsByName(elementName) {
    return NodeList.createImpl([], {
      element: this,
      query: () =>
        this._names[elementName] && this._names[elementName].length > 0 ? this._names[elementName] : []
    });
  }
